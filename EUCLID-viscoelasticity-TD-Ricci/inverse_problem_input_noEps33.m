...
tht_test = zeros(1,Nel,length(time));
    ...
    for ie=1:Nel
        ...
        epsvG(1:3,1:NMeG,ie,nt)     =        repmat(dt./(2*tauG + dt),1,3)'.*epsG(:,ie) +       betGnm1(1:3,1:NMeG,ie);
...
for nt=2:length(time)
    ...
    for ie=1:Nel
        ...
        betGnm1(1:3,1:NMeG,ie)             =   repmat(dt./(2*tauG + dt),1,3)'.*(epsGnm1 - epsvGnm1) + repmat(2*tauG./(2*tauG + dt),1,3)'.*epsvGnm1;
