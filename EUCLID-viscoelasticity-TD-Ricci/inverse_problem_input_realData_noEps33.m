function [betGnm1,betKnm1] = inverse_problem_input_realData_noEps33(nameDir,time,dt,coord,U,conne,Nel,NMeG,NMeK,tauG,tauK)
% addpath(genpath('routines'));dt(nt)(nt)
...
Stest(1).betGnm1  =   zeros(3,length(tauG),Nel);    % Beta parameters for the Shear moduli 
...
deformation_x=U(1:NN,nt);
deformation_y=U(NN+1:end,nt);

Z=zeros(2*NN,1);
Z(1:2:2*NN-1)=deformation_x;
Z(2:2:2*NN)=deformation_y;

epsG_test = zeros(3,Nel,length(time));
tht_test = zeros(1,Nel,length(time));
    ...
    for ie=1:Nel
        ...
        betGnm1(1:3,1:NMeG,ie) =  Stest(nt).betGnm1(1:3,1:NMeG,ie);
        betKnm1(1,1:NMeK,ie)   =  Stest(nt).betKnm1(1,1:NMeK,ie);

        %------------ Update viscous deformations -------------------------

        epsG(:,ie)                  =        epsG_test(:,ie,1); % St(nt).epsG(:,ie); % 
        epsvG(1:3,1:NMeG,ie,nt)     =        repmat(dt(nt)./(2*tauG + dt(nt)),1,3)'.*epsG(:,ie) +       betGnm1(1:3,1:NMeG,ie);
        
        tht(:,ie)                   =        tht_test(:,ie,1); % St(nt).tht(:,ie); % 
        thtv(1,1:NMeK,ie,nt)        =        (dt(nt)./(2*tauK + dt(nt)))'.*tht(:,ie)            +       betKnm1(1,1:NMeK,ie) ;

        %------------ Store variables -------------------------------------
        
        Stest(nt).epsvG(1:3,1:NMeG,ie)      =       epsvG(1:3,1:NMeG,ie,nt);
...
for nt=2:length(time)
    ...
    for ie=1:Nel
        ...
        epsGnm1                            =   epsG_test(1:3,ie,nt-1); % St(nt-1).epsG(1:4,ie); % 
        epsvGnm1                           =   Stest(nt-1).epsvG(1:3,1:NMeG,ie);
        betGnm1(1:3,1:NMeG,ie)             =   repmat(dt(nt)./(2*tauG + dt(nt)),1,3)'.*(epsGnm1 - epsvGnm1) + repmat(2*tauG./(2*tauG + dt(nt)),1,3)'.*epsvGnm1;

        thtnm1                             =   tht_test(1,ie,nt-1); % St(nt-1).tht(1,ie); % 
        thtvnm1                            =   Stest(nt-1).thtv(1,1:NMeK,ie);
        betKnm1(1,1:NMeK,ie)               =   (dt(nt)./(2*tauK + dt(nt)))'.*(thtnm1 - thtvnm1)             + (2*tauK./(2*tauK + dt(nt)))'.*thtvnm1;

        %------------- Update viscous deformations ------------------------

        epsG(:,ie)                     =       epsG_test(1:3,ie,nt); % St(nt).epsG(:,ie);
        epsvG(1:3,1:NMeG,ie,nt)        =       repmat(dt(nt)./(2*tauG + dt(nt)),1,3)'.*epsG(:,ie)          +       betGnm1(1:3,1:NMeG,ie);
        ...
        Stest(nt).betGnm1(1:3,1:NMeG,ie)   =   betGnm1(1:3,1:NMeG,ie);
