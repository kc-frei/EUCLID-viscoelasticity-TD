function [betGnm1,betKnm1] = inverse_problem_input_realData(nameDir,time,dt,coord,U,conne,Nel,NMeG,NMeK,tauG,tauK)
...
tht_test = zeros(1,Nel,length(time));
    ...
    for ie=1:Nel
        ...
        epsvG(1:4,1:NMeG,ie,nt)     =        repmat(dt(nt)./(2*tauG + dt(nt)),1,4)'.*epsG(:,ie) +       betGnm1(1:4,1:NMeG,ie);
...
for nt=2:length(time)
    ...
    for ie=1:Nel
        ...
        betGnm1(1:4,1:NMeG,ie)             =   repmat(dt(nt)./(2*tauG + dt(nt)),1,4)'.*(epsGnm1 - epsvGnm1) + repmat(2*tauG./(2*tauG + dt(nt)),1,4)'.*epsvGnm1;
